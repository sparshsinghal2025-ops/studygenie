...
document.getElementById('q').addEventListener('input', (e)=>{
  ...
  if(t.length>2){
    let isCode = checkIsCodingTopic(t);
    badge.innerText = isCode? '💻 CODE MODE' : '📚 THEORY MODE';
...
async function ask(){ if(!localStorage.getItem('genie_name')||!localStorage.getItem('genie_phone')){checkOnboard();return;} let input=document.getElementById('q'); let q=input.value.trim(); if(!q)return; if(!isDev && stats.wishes>=10){openPay();return;}
  ...
  let modeBadge = isCode? '<span class="bg-green-500 text-black text-[9px] px-2 py-0.5 rounded-full ml-2">💻 CODE+DRY RUN</span>' : '<span class="bg-white/20 text-[9px] px-2 py-0.5 rounded-full ml-2">📚 THEORY</span>';
  chat.innerHTML+=`<div class="flex justify-end hitpop"><div class="bubble-user px-4 py-2 text-[14px] mono">${q} ${modeBadge}</div></div>`;
  input.value='';
